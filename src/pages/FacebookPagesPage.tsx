...
export default function FacebookPagesPage() {
  ...
  return (
    <div className="max-w-6xl space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {pages.map((c) => (
          <Link key={c.id} to={`/facebook/${c.id}`} className="sd-card sd-card-link p-4 flex items-center gap-3.5">
            <div className="sd-icon-badge w-11 h-11 bg-blue-50 text-blue-700 shrink-0">
              <Facebook className="w-5 h-5" />
            </div>
            <div className="min-w-0">
              <p className="font-semibold text-sm text-[var(--sd-ink)] truncate">{c.name}</p>
              {c.config?.facebookPageName && (
                <p className="text-xs text-[var(--sd-muted)] truncate">{c.config.facebookPageName}</p>
              )}
            </div>
          </Link>
        ))}
